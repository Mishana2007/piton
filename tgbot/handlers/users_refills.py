...
async def success_refill(call: CallbackQuery, way, amount, id, user_id, pay_amount):
    try:
        ...
        msg = f"💰 Произошло пополнение баланса! \n" \
              ...
              f"🕒 Дата и время: {strftime('%d.%m.%Y %H:%M')}"

        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.topup_logs_msg_thr_id))
