...
async def mail_start_text(call: CallbackQuery, msg):
    admin_msg = f"<b>❗ Администратор @{call.from_user.username} запустил рассылку!</b>"
    await bot.send_message(chat_id=int(config.group_id), text=admin_msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
...
async def contest_create(call: CallbackQuery, state: FSMContext):
    ...
    await call.answer('Розыгрыш был запущен!')
...
async def mail_text_start(message: Message, state: FSMContext):
    ...
    await message.answer("<b>❗ Вы точно хотите запустить рассылку с таким текстом?</b>")
...
async def _mail_buttons_(call: CallbackQuery):
    ...
    if typ == 'link':
        ...
        if len(await db.get_all_pod_categories()) < 1:
            await call.answer("❗ Нет под-категорий для открытия", True)
        else:
            if len(await db.get_all_categories()) < 1:
                await call.answer("❗ Нет категорий для открытия под-категории", True)
            else:
...
async def edit_pos_open(call: CallbackQuery):
    ...
    if len(await db.get_pod_categories(cat_id)) != 0:
        await call.message.edit_text("<b>❗ Выберите под-категорию или позицию для ее открытия</b>",
                                reply_markup=await open_pod_cats_for_pos_add_mail(cat_id))
    else:
        await call.message.edit_text("<b>❗ Выберите позицию для открытия:</b>",
...
async def mail_photo_start(message: Message, state: FSMContext):
    msg = message.parse_entities()
    await message.answer("<b>❗ Отправьте фото для рассылки</b>")
...
async def settings_faq_set(message: Message, state: FSMContext):
    ...
    msg = f"<b>❗ Администратор  @{message.from_user.username} Изменил FAQ на: \n{message.parse_entities(as_html=True)}</b>"
...
async def settings_ref_per_set(message: Message, state: FSMContext):
    ...
    msg  = f"<b>❗ Администратор  @{message.from_user.username} изменил процент для {lvl} реферального уровня на: \n{message.text}</b>"
    await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
    await send_admins(
        msg,
        True)
...
async def settings_sup_set(message: Message, state: FSMContext):
    ...
    if message.text.startswith("https://t.me/") or message.text == "-":
        ...
        msg = f"<b>❗ Администратор  @{message.from_user.username} изменил Тех. Поддержку на: \n{message.text}</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        await send_admins(
            msg, True)
...
async def settings_chat_set(message: Message, state: FSMContext):
    ...
    if message.text.startswith("https://t.me/") or message.text == "-":
        ...
        msg = f"<b>❗ Администратор  @{message.from_user.username} изменил Чат на: \n{message.text}</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        await send_admins(
            msg, True
        )
...
async def settings_news_set(message: Message, state: FSMContext):
    ...
    if message.text.startswith("https://t.me/") or message.text == "-":
        ...
        msg = "<b>❗ Администратор  @{message.from_user.username} изменил Новостной канал на: \n{message.text}</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        await send_admins(
            msg, True
        )
...
async def pr_buttons(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text('<b>❗ Выберите действие:</b>', reply_markup=pr_buttons_inl())
...
async def pr_buttons2(c: CallbackQuery, state: FSMContext):
    ...
    if c.data.split(':')[1] == 'create':
        await c.message.edit_text('<b>❗ Введите название кнопки:</b>', reply_markup=pr_buttons_back())
...
async def extra_settings(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>❗ Выберите действие:</b>", reply_markup=extra_settings_inl())
...
async def here_name_promo(msg: Message, state: FSMContext):
    ...
    await msg.answer("<b>❗ Введите кол-во использований</b>")
...
async def here_discount_promo(msg: Message, state: FSMContext):
    if msg.text.isdigit():
        ...
        msg = f"<b>❗ Администратор  @{msg.from_user.username} создал Промокод <code>{name}</code> с кол-вом использований <code>{uses}</code> и скидкой <code>{int(msg.text)}{currencies[s['currency']]['sign']}</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        await send_admins(
            msg,
            True
...
async def promo_delete(msg: Message, state: FSMContext):
    try:
        ...
        msg = f"<b>❗ Администратор  @{msg.from_user.username} удалил Промокод <code>{msg.text}</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        await send_admins(
            msg, True
        )
...
async def here_count_lvl_ref(msg: Message, state: FSMContext):
    if msg.text.isdigit():
        ...
        msg = f"<b>❗ Администратор  @{msg.from_user.username} изменил кол-во рефералов для <code>{lvl}</code> уровня на <code>{count} чел</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        await send_admins(
            msg,
            True
...
async def user_balance_add(call: CallbackQuery, state: FSMContext):
    ...
    if action == "balance_add":
        await call.message.edit_text("<b>💰 Введите сумму, которую хотите выдать:</b>")
        ...
        await call.message.edit_text("<b>💰 Введите сумму, на которую хотите изменить:</b>")
...
👥 Рефералов: <code>{ref_count} чел</code></b>"""
        ...
        await call.message.edit_text("<b>❗ Введите сообщение, которое хотите отправить пользователю</b>")
