...
🕒 Дата и время: {strftime("%d.%m.%Y %H:%M")}
🎲 Содержимое товара:
{tovs}"""

                    await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.purchase_logs_msg_thr_id))
