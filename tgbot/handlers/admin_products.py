...
from tgbot.keyboards.inline_admin import (
    products_edits,
    back_pr_edits,
    open_cats_for_add_items,
    open_cats_for_add_pod_cat,
    open_cats_for_add_pos,
    open_cats_for_edit,
    open_cats_for_edit_pod_cat,
    open_cats_for_edit_pos,
    open_pod_cats_for_add_items,
    open_pod_cats_for_add_pos,
    edit_cat_inl,
    choose_clear_items_pos,
    choose_del_all_cats,
    choose_del_all_items,
    choose_del_all_pod_cats,
    choose_del_all_pos,
    choose_del_cat,
    choose_del_pod_cat,
    choose_del_pos,
    open_pod_cats_for_edit,
    open_pod_cats_for_edit_pos,
    open_positions_for_add_items,
    open_positions_for_edit,
    update_pod_cat_inl,
    edit_pos_inl,
    stop_add_items,
...
from tgbot.states.admin import (
    AdminCatsEdit,
    AdminPodCatsEdit,
    AdminPosEdit,
    AdminItemsEdit,
...
async def add_cat_op(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>❗ Введите название категории</b>", reply_markup=back_pr_edits()
    )
...
async def here_name_add_cat(msg: Message, state: FSMContext):
    ...
    msg = f"<b>❗ Администратор @{msg.from_user.username} создал категорию <code>{name}</code>!</b>"
    await bot.send_message(
        chat_id=int(config.group_id),
        text=msg,
        parse_mode="HTML",
        message_thread_id=int(config.admin_logs_msg_thr_id),
    )
...
async def edit_cat_op(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_all_categories()) < 1:
        await call.answer("❗ Нет категорий для изменения")
    else:
        await call.message.answer(
            "<b>❗ Выберите категорию для изменения:</b>",
            reply_markup=await open_cats_for_edit(),
        )
...
async def edit_cat_ope(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"<b>Категория: {category['name']}</b>", reply_markup=edit_cat_inl(cat_id)
    )
...
async def here_edit_name_cat(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"<b>✅ Название для категории изменено на <code>{msg.text}</code></b>"
    )
    msg = f"<b>❗ Администратор @{msg.from_user.username} изменил имя категории с <code>{cat['name']}</code> на <code>{msg.text}</code>!</b>"
    await bot.send_message(
        chat_id=int(config.group_id),
        text=msg,
        parse_mode="HTML",
...
async def del_cat_op(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>❓ Вы точно хотите удалить эту категорию?</b>",
        reply_markup=choose_del_cat(cat_id),
    )
...
async def del_cat_ope(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.answer(
            f"<b>✅ Вы успешно удалили категорию <code>{name}</code></b>"
        )
        msg = f"<b>❗ Администратор @{call.from_user.username} удалил категорию <code>{name}</code>!</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg,
        #     True,
        # )
    else:
        await call.message.answer("<b>❌ Вы отменили удаление категории</b>")
...
async def del_all_cats_op(call: CallbackQuery, state: FSMContext):
    await state.finish()
    await call.message.answer(
        "<b>❓ Вы точно хотите удалить <u>ВСЕ</u> категории?</b>",
        reply_markup=choose_del_all_cats(),
    )
...
async def dels_all_cats_choose(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.answer("<b>✅ Вы успешно удалили все категории</b>")
        msg = f"<b>❗ Администратор @{call.from_user.username} удалил ВСЕ категории!</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg,
        #     True,
        ...
        await call.message.answer("<b>❌ Вы отменили удаление всех категорий</b>")
...
async def add_cat_opee(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_all_categories()) < 1:
        await call.answer("❗ Нет категорий для создания под-категории")
    else:
        await call.message.answer(
            "<b>❗ Выберите категорию для создания под-категории</b>",
            reply_markup=await open_cats_for_add_pod_cat(),
        )
...
async def add_cat_openn(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>❗ Введите название под-категории</b>", reply_markup=back_pr_edits()
    )
...
async def here_name_add_cat(msg: Message, state: FSMContext):
    ...
    msg = f"<b>❗ Администратор @{msg.from_user.username} создал под-категорию <code>{msg.text}</code>!</b>"
    await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
    # await send_admins(
    #     msg,
    #     True,
...
async def edit_cat_op(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_all_pod_categories()) < 1:
        await call.answer("❗ Нет под-категорий для изменения")
    else:
        if len(await db.get_all_categories()) < 1:
            await call.answer("❗ Нет категорий для изменения под-категории")
        else:
            await call.message.answer(
                "<b>❗ Выберите категорию для изменения под-категории:</b>",
                reply_markup=await open_cats_for_edit_pod_cat(),
            )
...
async def pods_cat_edittt(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_pod_categories(cat_id)) < 1:
        await call.answer("❗ В этой категории нет под-категорий!")
    else:
        await call.message.answer(
            "<b>❗ Выберите под-категорию для изменения:</b>",
            reply_markup=await open_pod_cats_for_edit(cat_id),
        )
...
async def podss_cat_editt(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"<b>💎 Под-категория: <code>{pod_cat['name']}</code> \n⚙️ Категория: <code>{cat['name']}</code></b>",
        reply_markup=update_pod_cat_inl(pod_cat_id),
    )
...
async def edit_pod_cat_name(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("<b>❗ Введите новое название для под-категории</b>")
...
async def here_new_name_pod_cat(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"<b>✅ Вы успешно изменили название под-категории на <code>{msg.text}</code></b>"
    )
    msg = f"<b>❗ Администратор @{msg.from_user.username} изменил имя под-категории с <code>{cat['name']}</code> на <code>{msg.text}</code>!</b>"
    await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
    # await send_admins(
    #     msg,
    #     True,
...
async def del_pod_cat(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>❓ Вы точно хотите удалить под категорию?</b>",
        reply_markup=choose_del_pod_cat(pod_cat_id),
    )
...
async def del_pod_cat_yes_no(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.answer(
            f"<b>✅ Вы удалили под-категорию <code>{pod_cat['name']}</code></b>"
        )
        msg  = f"<b>❗ Администратор @{call.from_user.username} удалил под-категорию <code>{pod_cat['name']}</code>!</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg,
        #     True,
        ...
        await call.message.answer("<b>❌ Вы отменили удаление под-категории</b>")
...
async def del_all_pods_cats(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>❓ Вы точно хотите удалить <u>ВСЕ</u> под-категории?</b>",
...
async def dels_all_cats_choose(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.answer("<b>✅ Вы успешно удалили все под-категории</b>")
        msg = f"<b>❗ Администратор @{call.from_user.username} удалил ВСЕ под-категории!</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg,
        #     True,
        ...
        await call.message.answer("<b>❌ Вы отменили удаление всех под-категорий</b>")
...
async def add_pos(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>❗ Выберите категорию для создания позиции</b>",
        reply_markup=await open_cats_for_add_pos(),
    )
...
async def add_posss(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_pod_categories(cat_id)) != 0:
        await call.message.answer(
            "<b>❗ Выберите под-категорию (Или категорию) для создания позиции</b>",
            reply_markup=await open_pod_cats_for_add_pos(cat_id),
        )
    else:
        await call.message.answer("<b>❗ Введите название для позиции</b>")
...
async def here_name_addd_pos(msg: Message, state: FSMContext):
    ...
    await msg.answer("<b>❗ Введите цену для позиции</b>")
...
async def here_name_addd_pos(msg: Message, state: FSMContext):
    if msg.text.isdigit() or msg.text.replace(".", "").isdigit():
        ...
        await msg.answer("<b>❗ Цена должна быть числом!</b>")


@dp.callback_query_handler(
    text_startswith="position_type:", state=AdminPosEdit.here_type_add_pos
)
...
async def here_name_addd_pos(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "<b>❗ Отправьте <code>+</code> если хотите чтоб товар был бесконечным \nЕсли не хотите введите <code>-</code></b>"
    )
...
💚 Описание: <code>{desc}</code></b>
                    """
            msg1= f"<b>❗ Администратор @{msg.from_user.username} создал позицию: \n{msgg}</b>"
            await bot.send_message(chat_id=int(config.group_id), text=msg1, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
            # await send_admins(
            #     msg1,
            #     True,
            # )
...
💚 Описание: <code>{desc}</code></b>
            """
            msg2 = f"<b>❗ Администратор @{msg.from_user.username} создал позицию: \n{msgg}</b>"
            await bot.send_message(chat_id=int(config.group_id), text=msg2, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
            # await send_admins(
            #     msg2,
            #     True,
            # )
        if photo == "-":
            ...
            await bot.send_photo(
                ...
                photo=photo,
                caption=msgg,
                reply_markup=edit_pos_inl(pos_id),
            )
...
async def edit_pos_choose(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_all_categories()) < 1:
        ...
        if len(await db.get_all_positions()) < 1:
            ...
            await call.message.edit_text(
                "<b>❗ Выберите категорию для изменения позиции:</b>",
                reply_markup=await open_cats_for_edit_pos(),
            )
...
async def edit_pos_open(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_pod_categories(cat_id)) != 0:
        await call.message.edit_text(
            "<b>❗ Выберите под-категорию (Или категорию) для изменения позиции</b>",
            reply_markup=await open_pod_cats_for_edit_pos(cat_id),
        )
    else:
        await call.message.edit_text(
            "<b>❗ Выберите позицию для изменения:</b>",
            reply_markup=await open_positions_for_edit(cat_id),
        )
...
async def here_new_price(msg: Message, state: FSMContext):
    ...
    if msg.text.isdigit():
        ...
        curr = currencies[settings["currency"]]["sign"]
        await msg.answer(
            f"<b>✅ Вы изменили цену позиции на <code>{msg.text}{curr}</code></b>"
        )
        msg1 = f"<b>❗ Администратор @{msg.from_user.username} изменил цену позиции на <code>{msg.text}{curr}</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg1, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg1,
        #     True,
...
async def here_new_price(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"<b>✅ Вы изменили название позиции на <code>{msg.text}</code></b>"
    )
    msg1 = f"<b>❗ Администратор @{msg.from_user.username} изменил название позиции на <code>{msg.text}</code></b>"
    await bot.send_message(chat_id=int(config.group_id), text=msg1, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
    # await send_admins(
    #     msg1,
    #     True,
...
async def here_new_price(msg: Message, state: FSMContext):
    ...
    msg1 = f"<b>❗ Администратор @{msg.from_user.username} изменил описание позиции на</b> \n{desc}"
    await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
    # await send_admins(
    #     msg1,
    #     True,
...
async def here_new_price(msg: Message, state: FSMContext):
    ...
    if photo == "-":
        await msg.answer("<b>✅ Вы убрали фото позиции</b>")
        msg1 = f"<b>❗ Администратор @{msg.from_user.username} убрал фото у позиции: <code>{pos['name']}</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg1, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg1,
        #     True,
        ...
        msg2 = f"<b>❗ Администратор @{msg.from_user.username} изменил фото позиции <code>{pos['name']}</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg2, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg2,
        #     True,
        # )
        await msg.answer("<b>✅ Вы изменили фото позиции на</b>")
...
async def here_new_price(msg: Message, state: FSMContext):
    ...
    if msg.text == "+":
        await msg.answer(
            "<b>✅ Вы изменили тип товаров позиции на <code>Бесконечный</code></b>"
        )
        msg1 = f"<b>❗ Администратор @{msg.from_user.username} изменил тип товаров позиции <code>{pos['name']}</code> на <code>Бесконечный</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg1, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg1,
        #     True,
        ...
        msg2 = f"<b>❗ Администратор @{msg.from_user.username} изменил тип товаров позиции <code>{pos['name']}</code> на <code>Обычный</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg2, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg2,
        #     True,
        # )
        await msg.answer(
            "<b>✅ Вы изменили тип товаров позиции на <code>Обычный</code></b>"
        )
...
async def edit_del_pos(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer(
        "<b>❓ Вы точно хотите удалить позицию?</b>",
        reply_markup=choose_del_pos(pos_id),
    )
...
async def dels_pos(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.delete()
        await call.message.answer(
            f"<b>✅ Вы удалили позицию <code>{pos['name']}</code></b>"
        )
        msg = f"<b>❗ Администратор @{call.from_user.username} удалил позицию <code>{pos['name']}</code></b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg,
        #     True,
        ...
        await call.message.delete()
        await call.message.answer("<b>❌ Вы отменили удаление позиции</b>")
...
async def del_all_poss(call: CallbackQuery, state: FSMContext):
    await state.finish()
    await call.message.edit_text(
        "<b>❓ Вы точно хотите удалить <u>ВСЕ</u> позиции?</b>",
        reply_markup=choose_del_all_pos(),
    )
...
async def del_all_posss(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.edit_text("<b>✅ Вы удалили все позиции</b>")
        msg = f"<b>❗ Администратор @{call.from_user.username} удалил ВСЕ позиции</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg,
        #     True,
...
async def edit_clear_it_pos(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer(
        "<b>❓ Вы точно хотите очистить товары позиции?</b>",
        reply_markup=choose_clear_items_pos(pos_id),
    )
...
async def clear_itemss(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.delete()
        await call.message.answer("<b>✅ Вы очистили товары позиции</b>")
    else:
        await call.message.delete()
        await call.message.answer("<b>❌ Вы отменили чистку товаров позиции</b>")
...
❗ Загружайте по одному</b>"""
    ...
    await call.message.answer(msg, reply_markup=back_pr_edits())
...
async def edit_pos_choose(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_all_categories()) < 1:
        ...
        if len(await db.get_all_positions()) < 1:
            ...
            await call.message.edit_text(
                "<b>❗ Выберите категорию для добавления товаров:</b>",
                reply_markup=await open_cats_for_add_items(),
            )
...
async def edit_pos_open(call: CallbackQuery, state: FSMContext):
    ...
    if len(await db.get_pod_categories(cat_id)) != 0:
        await call.message.edit_text(
            "<b>❗ Выберите под-категорию (Или категорию) для добавления товаров</b>",
            reply_markup=await open_pod_cats_for_add_items(cat_id),
        )
    else:
        await call.message.edit_text(
            "<b>❗ Выберите позицию для добавления товаров:</b>",
            reply_markup=await open_positions_for_add_items(cat_id),
        )
...
@dp.message_handler(
    content_types=["photo", "document"], state=AdminItemsEdit.here_data_items
)
...
async def here_data_items(message: Message, state: FSMContext):
    ...
    if _type == "text":
        ...
        return await cache_msg.edit_text("<b>❗ Отправьте нужный тип товара!</b>")

    await cache_msg.edit_text(
        f"<b>✅ Товары в кол-ве <code>{count_add}шт</code> были успешно добавлены</b>",
...
async def del_all_itemss(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "<b>❓ Вы точно хотите удалить <u>ВСЕ</u> товары?</b>",
        reply_markup=choose_del_all_items(),
    )
...
async def dels_all_items(call: CallbackQuery, state: FSMContext):
    ...
    if action == "yes":
        ...
        await call.message.edit_text("<b>✅ Вы удалили все товары</b>")
        msg = f"<b>❗ Администратор @{call.from_user.username} удалил ВСЕ товары</b>"
        await bot.send_message(chat_id=int(config.group_id), text=msg, parse_mode="HTML", message_thread_id=int(config.admin_logs_msg_thr_id))
        # await send_admins(
        #     msg,
        #     True,
